...
class Bot:
    ...
    def get_move(self, state):
        ...
        if bestRoot is None:
            ...
            if self.root.state.get_opponents_played_card() is not None:
                ...
                print("Opponent Played this: ", self.root.card[0],self.root.card[1])
    ...
    def MCTS(self,state):
        for iteration in range(0, self.DL):
            if iteration == 0:
                ...
                search = True
                crNode = self.root
                while search and crNode.state.finished():
                    crNode = crNode.children[self.selection(crNode)]
                    
                    if len(crNode.children) > 0:
                        search = True
                    else:
                        #if crNode.state.finished() is not True:
        ...
        for child in self.root.children:
            if child.n is 0 or child.v is 0:
                score = 0
            else:
    ...
    def selection(self, currentNode = Node(), C = 2):
        t = self.root.n
